...
<section class="content" style="margin-left: 20px;margin-right: 20px;">
  <div class="row">
      <div class="col-sm-12" >
         {{ Form::open(['url' => 'SaiAutoCare/workshopWTax/search','files' => 'true' ,'enctype' => 'multipart/form-data', 'autocomplete' => 'OFF']) }} 
        <div class="card">
          <div class="card-body">
    ...
    <div class="col-sm-12" id="HideForShowProduct">
        <div class="card">
          <div class="card-header">
            <i class="fa fa-align-justify"></i> Workshop Detail for Without GST
          </div>
                ...
                <tr>
                 ...
                 <!--  <td>{{ $value['vin'] }}</td>
                  ...
                  <td style="white-space: nowrap">
                    ...
                    <a target="blank" href="{{ url('/')}}/SaiAutoCare/workshopWTax/view/{{ $value['id'] }}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                    @if($value['is_workshop']==1)
                   <a  href="{{ url('/')}}/SaiAutoCare/workshopWTax/add/{{ $value['id'] }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a> 
                   @else
                   <a style="display: none" data-toggle="modal" id="{{ $value['id'] }}" data-target="#myModal1"  class="btn btn-success openPayentModel btn-sm"><i class="fa fa-undo" aria-hidden="true"></i></a> 
                     ...
                     @endif
                     @if($role_id==1)
                      <a style="display: none" href="{{ url('/')}}/SaiAutoCare/workshopWTax/trash/{{ $value['id']}} " class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this user?');"><i class="fa fa-remove"></i></a>
                      @endif
                  </td>
                </tr>  
                @endforeach 
     ...
     $(document).on('click','#paymentForWorkshop',function()
    {
        ...
        var commentsForWorkshop=$('[name^=commentsForWorkshop]').val();
           ...
           else
           {
               $.ajax({
              type:"POST",
              url: "{{  url('/') }}/ajax/paymentForWorkshopwithoutGst",
              data:{
              "_token": "{{ csrf_token() }}",
              ...
              success: function(data){
                 ...
                 if(data==1)
                 {
                  swal("Good job!", "Workshop Payment  Successfully", "success");
                      $('[name=amountForWorkshop]').val("");
                       var newTab = window.open("{{url('/')}}/SaiAutoCare/workshopWTax/view/"+workshopIdForPayment, "_blank");
                      newTab.location;
                   console.log("Worshop Detail Opened In New Tab");
                 }
                 else if (data==0){
