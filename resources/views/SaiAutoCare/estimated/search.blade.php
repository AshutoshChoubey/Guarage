...
<section class="content" style="margin-left: 20px;margin-right: 20px;">
  <div class="row">
      <div class="col-sm-12" >
         {{ Form::open(['url' => 'SaiAutoCare/estimation/search','files' => 'true' ,'enctype' => 'multipart/form-data', 'autocomplete' => 'OFF']) }} 
        <div class="card">
    ...
    <div class="col-sm-12" id="HideForShowProduct">
        <div class="card">
          <div class="card-header">
            <i class="fa fa-align-justify"></i> Workshop Estimated Detail 
          </div>
                ...
                <tr>
                 ...
                 <!--  <td>{{ $value['vin'] }}</td>
                  ...
                  <td style="white-space: nowrap">
                     <a style="display: none" data-toggle="modal" id="{{ $value['id'] }}" data-target="#workshopDiscount"  class="btn btn-success openDiscountModelForWorkshop btn-sm"><i class="fa fa-wpexplorer" aria-hidden="true"></i>Discount</i></a> 
                       {{-- <a id="{{ $value['id'] }}"  class="btn btn-success openRefreshBalance btn-sm"><i class="fa fa-refresh" aria-hidden="true"></i></i>Balance</i></a>  --}}
                    
                       <a style="display: none" data-toggle="modal" id="{{ $value['id'] }}" data-target="#workshopPayment"  class="btn btn-success openPayentModelForWorkshop btn-sm"><i class="fa fa-credit-card" aria-hidden="true"></i></a> 

                    <a target="blank" href="{{ url('/')}}/SaiAutoCare/estimation/view/{{ $value['id'] }}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                    @if($value['is_workshop']==1)
                   <a href="{{ url('/')}}/SaiAutoCare/estimation/add/{{ $value['id'] }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a> 
                   @else
                   <a style="display: none" data-toggle="modal" id="{{ $value['id'] }}" data-target="#myModal1"  class="btn btn-success openPayentModel btn-sm"><i class="fa fa-undo" aria-hidden="true"></i></a> 
                     <a style="display: none" href="{{ url('/')}}/SaiAutoCare/sale/edit/{{ $value['id'] }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a> 

                     @endif
                     @if($role_id==1)
                      <a style="display: none" href="{{ url('/')}}/SaiAutoCare/estimation/trash/{{ $value['id']}} " class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this user?');"><i class="fa fa-remove"></i></a>
                      @endif
                  </td>
                </tr>  
                @endforeach 
              ...
              success: function(data){
                 ...
                 if(data==1)
                 {
                  ...
                  $('[name=amountForWorkshopDiscount]').val("");      
                   var newTab = window.open("{{url('/')}}/SaiAutoCare/estimation/view/"+workshopIdForDiscount, "_blank");
                      newTab.location;
              ...
              success: function(data){
                 ...
                 if(data==1)
                 {
                  swal("Good job!", "Workshop Payment  Successfully", "success");
                      $('[name=amountForWorkshop]').val("");
                       var newTab = window.open("{{url('/')}}/SaiAutoCare/estimation/view/"+workshopIdForPayment, "_blank");
                      newTab.location;
                   console.log("Worshop Detail Opened In New Tab");
                 }
                 else if (data==0){
