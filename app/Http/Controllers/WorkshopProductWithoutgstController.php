...
use App\Service; 
use DB; 
use App\WorkshopProductsWithoutgst;
use App\WorkshopServicesWithoutgst;
use App\Modal;
use App\Brand;
use App\ServiceType;
use App\HeaderLink;
...
use Auth;
use App\WorkshopProductsEstimatedsWithoutgst;

class WorkshopProductWithoutgstController extends Controller
{
    ...
    public function save(Request $request, $id = null)
    {
        ...
        if(isset($id) && $id != null ){
            $getFormAutoFillup = WorkshopsWithoutgst::whereId($id)->first()->toArray();
              $viewData['workshopProductData']= WorkshopProductsWithoutgst::where('workshop_id','=',$id)->get();
              $viewData['workshopServiceData']= WorkshopServicesWithoutgst::where('workshop_id','=',$id)->get();
            return view('SaiAutoCare.workshopWithoutGst.add', $viewData)->with($getFormAutoFillup);
        }
        else if((!isset($id) && $id == null) && !$request->isMethod('post') )
        {
            return view('SaiAutoCare.workshopWithoutGst.add', $viewData);
        }
        else {
            ...
            if ($request->isMethod('post')){
                ...
                if(isset($request->id) && $request->id != null) 
                {
                    if ($request->isMethod('post')){
                        ...
                        if(WorkshopsWithoutgst::where([['id', '=', $request->id]])->update($PartyManage)){
 
                              $workshopProductData= WorkshopProductsWithoutgst::where('workshop_id','=',$request->id)->get();
                            ...
                            WorkshopProductsWithoutgst::where('workshop_id',$request->id)->forceDelete();
                            WorkshopServicesWithoutgst::where('workshop_id',$request->id)->forceDelete();
                             if(isset($request->product_id) && $request->product_id[0]!=null)
                            {
                                for($i=0; $i < count($request->product_id); $i++){
                                        $WorkshopProduct= new WorkshopProductsWithoutgst();
                        ...
                        for($i=0; $i < count($request->service_id); $i++){
                                $WorkshopService= new WorkshopServicesWithoutgst();
                    ...
                    $viewData['workshopId'] = $request->id;
                     return redirect('/SaiAutoCare/workshopWTax/add/'.$request->id);
                }
                else
                {
                    ...
                    $PartyManage = new WorkshopsWithoutgst($PartyManage);
                    if($PartyManage->save()){
                        ...
                        if(isset($request->product_id_es[0]) && $request->product_id_es[0]!='')
                        {
                          for($i=0; $i < count($request->product_id_es); $i++){
                                    $WorkshopProduct_es= new WorkshopProductsEstimatedsWithoutgst();
   ...
   // this is for search 
    public function view(Request $request)
    {
        ...
        if($request->isMethod('post')) 
        {
            ...
            $workshop= DB::table('workshops_withoutgsts');
            $workshop->leftJoin('modals','modals.id','=','workshops_withoutgsts.model_number');
            $workshop->leftJoin('brands','brands.id','=','workshops_withoutgsts.brand');
            $getFormAutoFillup=$request->all();
            $workshop->where('workshops_withoutgsts.deleted_at','=',null);
            if($request->has('id') && $request->id !=''){
                $workshop->where('workshops_withoutgsts.id', '=', $request->id);
            }
            if($request->has('customer_id') && $request->customer_id !=''){
                $workshop->where('workshops_withoutgsts.customer_id', '=', $request->customer_id);
            }
            if($request->has('created_at_from') && $request->created_at_from !=''){
                $workshop->whereDate('workshops_withoutgsts.created_at', '<=', $request->created_at_from);
            }
            if($request->has('created_at_to') && $request->created_at_to !=''){
                $workshop->whereDate('workshops_withoutgsts.created_at', '>=', $request->created_at_to);
            }
            if($request->has('mobile') && $request->mobile !=''){
                $workshop->where('workshops_withoutgsts.mobile', '=', $request->mobile);
            }
            if($request->has('email') && $request->email !=''){
                $workshop->where('workshops_withoutgsts.email', '=', $request->email);
            }
            if($request->has('vehicle_reg_number_for_search') && $request->vehicle_reg_number_for_search !=''){
                $workshop->where('workshops_withoutgsts.vehicle_reg_number', 'like', '%'.$request->vehicle_reg_number_for_search.'%');
            }
             if($request->has('brand') && $request->brand !=''){
                $workshop->where('workshops_withoutgsts.brand', '=', $request->brand);
            }
             if($request->has('model_number') && $request->model_number !=''){
                 $workshop->where('workshops_withoutgsts.model_number', '=', $request->model_number);
            }
            $workshop->select('workshops_withoutgsts.*','brands.brand_name as company_name_from_brand','modals.model_name as modelNumber');
            $workshop->orderBy('id','desc');
            $workshop= $workshop->get();
            $viewData['workshop']=json_decode(json_encode($workshop), true);
            return view('SaiAutoCare.workshopWithoutGst.search', $viewData)->with($getFormAutoFillup);
        }else
        {
            ...
            $workshop->where('workshops_withoutgsts.deleted_at','=',null);
            $workshop->select('workshops_withoutgsts.*','brands.brand_name as company_name_from_brand','modals.model_name as modelNumber');
            $workshop->orderBy('workshops_withoutgsts.id','desc');
            $workshop= $workshop->get();
            $viewData['workshop']=json_decode(json_encode($workshop), true);
        //  $workshop= DB::table('workshops');
            ...
            //$workshop= $workshop->get();
            //$viewData['workshop']=json_decode(json_encode($workshop), true);
            return view('SaiAutoCare.workshopWithoutGst.search', $viewData);
        }
    ...
    {
        $viewData['header_link'] =  HeaderLink::where("menu_id",'3')->select("link_title","link_name")->orderBy('id','desc')->get();
        $getIndivisualWorkshopDetail = WorkshopsWithoutgst::whereId($id)->first()->toArray();
        ...
        $workshop_products_estimateds = DB::table('workshop_products_estimateds_withoutgsts')
        ->join('products','products.id','=','workshop_products_estimateds_withoutgsts.product_id_es')
        ->select('products.product_name','workshop_products_estimateds_withoutgsts.product_quantity_es','workshop_products_estimateds_withoutgsts.product_price_es as ProductWorkshopPrice','products.hsn as ProductHsn','products.unit_price_exit as UnitExitPrice','products.gst as ProductGst','products.sgst as Productsgst','products.cgst as Productcgst','products.igst as Productigst','products.discount_value as discount_value')
        ->where('workshop_id_es',$getIndivisualWorkshopDetail['id'])->get();
         $WorkshopProduct = DB::table('workshop_products_withoutgsts')
        ->join('products','products.id','=','workshop_products_withoutgsts.product_id')
        ->select('products.id','products.product_name','workshop_products_withoutgsts.product_quantity','workshop_products_withoutgsts.product_price as ProductWorkshopPrice','products.hsn as ProductHsn','products.unit_price_exit as UnitExitPrice','products.gst as ProductGst','products.sgst as Productsgst','products.cgst as Productcgst','products.igst as Productigst','products.discount_value as discount_value')
        ->where('workshop_id',$getIndivisualWorkshopDetail['id'])->get();
        $WorkshopService = DB::table('workshop_services_withoutgsts')
        ->join('services','services.id','=','workshop_services_withoutgsts.service_id')
        ->where('workshop_id',$getIndivisualWorkshopDetail['id'])->get();
        ...
        $viewData['workshopId']="";
        return view('SaiAutoCare.workshopWithoutGst.view',$viewData)->with($getIndivisualWorkshopDetail);
    }
